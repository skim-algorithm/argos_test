...
class AroonIndicator(IndicatorMixin):
    ...
    def _run(self):
        ...
        self._aroon_up = rolling_close.apply(lambda x: float(np.argmax(x) + 1) / self._window * 100, raw=True)
        self._aroon_down = rolling_close.apply(lambda x: float(np.argmin(x) + 1) / self._window * 100, raw=True)
...
class WMAIndicator(IndicatorMixin):
    ...
    def _run(self):
        _weight = pd.Series([i * 2 / (self._window * (self._window + 1)) for i in range(1, self._window + 1)])
...
class CCIIndicator(IndicatorMixin):
    ...
    def _run(self):
        ...
        self._cci = (typical_price - typical_price.rolling(self._window, min_periods=min_periods).mean()) / (
            self._constant * typical_price.rolling(self._window, min_periods=min_periods).apply(_mad, True)
        )
...
class PSARIndicator(IndicatorMixin):
    ...
    def psar_up_indicator(self) -> pd.Series:
        ...
        Returns:
            pandas.Series: New feature generated.
        """
        indicator = self._psar_up.where(self._psar_up.notnull() & self._psar_up.shift(1).isnull(), 0)
    ...
    def psar_down_indicator(self) -> pd.Series:
        ...
        Returns:
            pandas.Series: New feature generated.
        """
        indicator = self._psar_up.where(self._psar_down.notnull() & self._psar_down.shift(1).isnull(), 0)
        indicator = indicator.where(indicator == 0, 1)
...
def vortex_indicator_pos(high, low, close, window=14, fillna=False):
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    return VortexIndicator(high=high, low=low, close=close, window=window, fillna=fillna).vortex_indicator_pos()
...
def vortex_indicator_neg(high, low, close, window=14, fillna=False):
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    return VortexIndicator(high=high, low=low, close=close, window=window, fillna=fillna).vortex_indicator_neg()
...
def cci(high, low, close, window=20, constant=0.015, fillna=False):
    ...
    """
    return CCIIndicator(high=high, low=low, close=close, window=window, constant=constant, fillna=fillna).cci()
...
def ichimoku_conversion_line(high, low, window1=9, window2=26, visual=False, fillna=False) -> pd.Series:
    """Tenkan-sen (Conversion Line)
...
def psar_up(high, low, close, step=0.02, max_step=0.20, fillna=False):
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    indicator = PSARIndicator(high=high, low=low, close=close, step=step, max_step=max_step, fillna=fillna)
