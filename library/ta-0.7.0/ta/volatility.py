...
class KeltnerChannel(IndicatorMixin):
    ...
    def _run(self):
        ...
        if self._original_version:
            self._tp = (
                ((self._high + self._low + self._close) / 3.0).rolling(self._window, min_periods=min_periods).mean()
            )
            self._tp_high = (
                (((4 * self._high) - (2 * self._low) + self._close) / 3.0).rolling(self._window, min_periods=0).mean()
            )
            self._tp_low = (
                (((-2 * self._high) + (4 * self._low) + self._close) / 3.0).rolling(self._window, min_periods=0).mean()
            )
        else:
...
def donchian_channel_hband(high, low, close, window=20, offset=0, fillna=False):
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    indicator = DonchianChannel(high=high, low=low, close=close, window=window, offset=offset, fillna=fillna)
