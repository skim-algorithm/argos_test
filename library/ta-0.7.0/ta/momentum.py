...
class RSIIndicator(IndicatorMixin):
    ...
    def _run(self):
        ...
        emaup = up_direction.ewm(alpha=1 / self._window, min_periods=min_periods, adjust=False).mean()
        emadn = down_direction.ewm(alpha=1 / self._window, min_periods=min_periods, adjust=False).mean()
...
class UltimateOscillator(IndicatorMixin):
    ...
    def _run(self):
        ...
        buying_pressure = self._close - pd.DataFrame({"low": self._low, "close": close_shift}).min(axis=1, skipna=False)
...
class KAMAIndicator(IndicatorMixin):
    ...
    def _run(self):
        ...
        smoothing_constant = (
            (efficiency_ratio * (2.0 / (self._pow1 + 1) - 2.0 / (self._pow2 + 1.0)) + 2 / (self._pow2 + 1.0)) ** 2.0
        ).values
        ...
        for i in range(len_kama):
            if np.isnan(smoothing_constant[i]):
                ...
                self._kama[i] = self._kama[i - 1] + smoothing_constant[i] * (close_values[i] - self._kama[i - 1])
...
class ROCIndicator(IndicatorMixin):
    ...
    def _run(self):
        self._roc = ((self._close - self._close.shift(self._window)) / self._close.shift(self._window)) * 100
...
class WilliamsRIndicator(IndicatorMixin):
    ...
    def _run(self):
        ...
        lowest_low = self._low.rolling(self._lbp, min_periods=min_periods).min()  # lowest low over lookback period lbp
...
class StochRSIIndicator(IndicatorMixin):
    ...
    def _run(self):
        ...
        self._stochrsi = (self._rsi - lowest_low_rsi) / (self._rsi.rolling(self._window).max() - lowest_low_rsi)
...
def tsi(close, window_slow=25, window_fast=13, fillna=False) -> pd.Series:
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    return TSIIndicator(close=close, window_slow=window_slow, window_fast=window_fast, fillna=fillna).tsi()
...
def stochrsi(
    ...
    Returns:
            pandas.Series: New feature generated.
    """
    return StochRSIIndicator(close=close, window=window, smooth1=smooth1, smooth2=smooth2, fillna=fillna).stochrsi()
...
def stochrsi_k(
    ...
    Returns:
            pandas.Series: New feature generated.
    """
    return StochRSIIndicator(close=close, window=window, smooth1=smooth1, smooth2=smooth2, fillna=fillna).stochrsi_k()
...
def stochrsi_d(
    ...
    Returns:
            pandas.Series: New feature generated.
    """
    return StochRSIIndicator(close=close, window=window, smooth1=smooth1, smooth2=smooth2, fillna=fillna).stochrsi_d()
...
def ppo_signal(close: pd.Series, window_slow=26, window_fast=12, window_sign=9, fillna=False) -> pd.Series:
    """
    The Percentage Price Oscillator (PPO) is a momentum oscillator that measures
    the difference between two moving averages as a percentage of the larger moving average.
